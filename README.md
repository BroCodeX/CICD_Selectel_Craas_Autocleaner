...
- Для образов, попавших под правило, защищает `keep_latest` последних и удаляет только те, что старше `remove_older` (в сутках). Опционально задается на уровне rules файла (секция cleanup_defaults).
- Для образов, не попавших ни под одно правило (`unmatched`), оставляет `keep_latest` последних и удаляет только те, что старше `remove_older` (в сутках). Опционально задается на уровне rules файла (секция unmatched_defaults).
